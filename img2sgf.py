...
from PIL import Image, ImageEnhance, ImageTk  # , ImageGrab -- Windows/Mac only!
...
def crop_and_rotate_image():
    ...
    region_PIL = input_image_PIL.rotate(
        angle=-rotate_angle.get(), fillcolor="white", center=rotation_centre
    ).crop(selection_global)


def process_image():
    global input_image_np, edge_detected_image_np, edge_detected_image_PIL, circles, circles_removed_image_np, circles_removed_image_PIL, grey_image_np, region_PIL
    ...
    draw_histogram(
        stone_brightnesses
    )  # this should erase the histogram from any previous board
...
def draw_histogram(stone_brightnesses):
    ...
    threshold_line = black_thresh_subfigure.plot(
        2 * [black_stone_threshold], [0, max_val], color="red"
    )
    black_thresh_subfigure.text(
        black_stone_threshold, max_val * 0.95, str(black_stone_threshold), fontsize=8
    )
    black_thresh_subfigure.text(
        black_stone_threshold - 70,
        max_val * 0.8,
        str(num_black_stones) + " black",
        fontsize=8,
    )
    black_thresh_subfigure.text(
        black_stone_threshold + 10,
        max_val * 0.8,
        str(num_white_stones) + " white",
        fontsize=8,
    )
    black_thresh_hist.draw()
    ...
    if len(hlines) > 0:
        ...
        if hclusters is not None:
            for i in range(len(hlines)):
                threshold_subfigure.plot(
                    ymin, hlines[i], color=colours[hclusters.labels_[i]], marker="."
                )
        for x in vcentres:
    ...
    if len(vlines) > 0:
        ...
        if vclusters is not None:
            for i in range(len(vlines)):
                threshold_subfigure.plot(
                    vlines[i], xmin, color=colours[vclusters.labels_[i]], marker="."
                )
        for y in hcentres:
...
def find_grid():
    global valid_grid, board_ready, circles, vsize, hsize, hspace, vspace, hcentres, vcentres, hcentres_complete, vcentres_complete
...
def initialise_parameters():
    ...
    global region_PIL, image_loaded, found_grid, valid_grid, board_ready, board_edited, board_alignment, previous_rotation_angle, black_stone_threshold, selection_global
...
def open_file(input_file=None):
    ...
    if len(input_file) > 0:
        ...
        try:
            ...
            mb.showinfo("Can't open file", input_file + " isn't a valid image file")
            return

        log(
            "Image size "
            + str(input_image_PIL.size[0])
            + "x"
...
def select_region():
    ...
    selection_global = np.array(
        (
            selection_global[0] + scale * sel_x1,
            selection_global[1] + scale * sel_y1,
            selection_global[0] + scale * sel_x2,
            selection_global[1] + scale * sel_y2,
        )
    )
    ...
    sel_rect_id = input_canvas.create_rectangle(
        0, 0, 0, 0, dash=(6, 6), fill="", outline="green", width=3
    )
    draw_images()
...
def draw_images():
    ...
    if show_circles.get() == 1:
        ...
        processed_photo, scale = scale_image(
            circles_removed_image_PIL, processed_canvas
        )
    ...
    if found_grid:
        ...
        if valid_grid:  # show the red lines where gaps have been filled
            for y in hcentres_complete:
                processed_canvas.create_line(
                    xmin, y * scale, xmax, y * scale, fill="red", width=2
                )
            for x in vcentres_complete:
                processed_canvas.create_line(
                    x * scale, ymin, x * scale, ymax, fill="red", width=2
                )
        # if not valid, still show green lines with the uneven spacing or bad gaps
        for y in hcentres:
            processed_canvas.create_line(
                xmin, y * scale, xmax, y * scale, fill="green", width=2
            )
        for x in vcentres:
            processed_canvas.create_line(
                x * scale, ymin, x * scale, ymax, fill="green", width=2
            )
    ...
    if not board_ready:
        if image_loaded:
            ...
            output_canvas.create_text(
                (0, 60), text="- Select a smaller region", anchor="nw"
            )
            ...
            output_canvas.create_text(
                (0, 150), text="  -> Increase contrast", anchor="nw"
            )
            output_canvas.create_text(
                (0, 180), text="  -> Increase threshold", anchor="nw"
            )
        return
...
def edit_board(event):
    ...
    if (
        cmin - grid_space / 2 < x < cmax + grid_space / 2
        and cmin - grid_space / 2 < y < cmax + grid_space / 2
    ):
        i, j = round((x - cmin) / (cmax - cmin) * 18), round(
            (y - cmin) / (cmax - cmin) * 18
        )
        ...
        if hsize < BOARD_SIZE and vsize < BOARD_SIZE:
            if not (
                cmin < x < cmax or cmin < y < cmax
            ):  # ignore clicks away from the corners
            ...
            vsize < BOARD_SIZE and c1 < x < c2
        ):  # only respond to click at top or bottom
            board_alignment[1] = Alignment.TOP if y < cmin else Alignment.BOTTOM
        elif (
            hsize < BOARD_SIZE and c1 < y < c2
        ):  # only respond to click at left or right
            board_alignment[0] = Alignment.LEFT if x < cmin else Alignment.RIGHT
...
input_canvas.bind(
    "<Configure>", lambda x: draw_images()
)  # also draw the processed image
...
input_instructions = tk.Label(
    input_frame, text="click and drag to zoom\ndouble-click to reset"
)
...
settings_button = tk.Button(
    processed_frame, text="show settings", command=toggle_settings
)
...
show_circles.set(1)
show_circles_button = tk.Checkbutton(
    processed_frame,
...
rotate_angle = tk.Scale(
    processed_frame, from_=-45, to=45, orient=tk.HORIZONTAL, length=image_size
)
...
sobel = tk.Scale(
    settings1, from_=3, to=7, orient=tk.HORIZONTAL, command=odd_only, length=100
)
...
gradientL1 = tk.Radiobutton(
    settings1, text="L1 norm", variable=gradient, value=1, command=process_image
)
# gradientL1.grid(row=13)
gradientL2 = tk.Radiobutton(
    settings1, text="L2 norm", variable=gradient, value=2, command=process_image
)
# gradientL2.grid(row=14)


threshold_label = tk.Label(
    settings2, text="line detection threshold\nfor Hough transform"
)
